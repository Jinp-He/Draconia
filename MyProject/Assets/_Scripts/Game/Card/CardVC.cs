...
namespace Draconia.ViewController
{
        ...
        IBeginDragHandler, IEndDragHandler
    {
        ...
        cardPosition = CardPosition.Battle)
        {
            ...
            //Debug.Log("Name" + _cardInfo.Name);
            CardImage.sprite = this.GetSystem<ResLoadSystem>().LoadSprite( _cardInfo.Name,"连抚式");
            ...
            {
                switch (_cardInfo.Rarity)
                {
                    case Rarity.Normal:
                        CardRarity.sprite = this.GetSystem<ResLoadSystem>().LoadSprite("低稀有度");
                        break;
                    case Rarity.Rare:
                        CardRarity.sprite = this.GetSystem<ResLoadSystem>().LoadSprite("中稀有度");
                        break;
                    case Rarity.Epic:
                        CardRarity.sprite = this.GetSystem<ResLoadSystem>().LoadSprite("高稀有度");
                        break;
                    case Rarity.Basic:
                        break;
                    default:
        ...
        public void OnDrag(PointerEventData eventData)
        {
            ...
            ChooseTarget(res);
            Card.ChooseTarget( res, Target, _enemies, _allies);
        ...
        protected void ChooseTarget(List<RaycastResult> res)
        {
            switch (_cardInfo.SkillTargetType)
            {
                ...
                case SkillTarget.Single:
                    if (res[0].gameObject.CompareTag("PlayerRaycast") ||
                        res[0].gameObject.CompareTag("EnemyRaycast"))
                    {
                        _enemies.Clear();
                        _allies.Clear();
                        if (res[0].gameObject.CompareTag("PlayerRaycast"))
        ...
        public virtual void Play()
        {
            ...
            CardUser.Player.CardUsageHistory.Add(this);
