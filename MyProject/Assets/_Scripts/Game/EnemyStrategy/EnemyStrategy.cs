...
namespace Draconia.ViewController
{
    public class EnemyStrategy : ICanGetSystem, ICanRegisterEvent, ISingleton
    {
        ...
        {
            _enemy = enemy;
            Attack = this.GetSystem<ResLoadSystem>().LoadSync<Sprite>("Intention_Attack");
            Skill = this.GetSystem<ResLoadSystem>().LoadSync<Sprite>("Intention_Skill");
            Other = this.GetSystem<ResLoadSystem>().LoadSync<Sprite>("Intention_Other");
            Move = this.GetSystem<ResLoadSystem>().LoadSync<Sprite>("Intention_Move");
        ...
        {
            Debug.Log("#BattleSystem# :" + enemy.Alias);
            return enemy.Alias switch
            {
                "dog01" => new Hounds(enemy),
                "fatknight" => new FatKnight(enemy),
                _ => new Hounds(enemy)
            };
        }
        ...
        protected void UpdateIntentionSpot(List<int> intentionSpot)
        {
            ...
            List<UnityEngine.UI.Image> positionMarker = _enemy.Intention.GetComponent<Intention>().PositionMarker
                .GetComponentsInChildren<UnityEngine.UI.Image>().ToList();
            for (int i = 0; i < 4; i++)
            {
